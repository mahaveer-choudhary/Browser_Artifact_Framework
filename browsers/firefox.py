...
class FirefoxExtractor:
    ...
    def extract_cookies(self) -> List[Dict]:
        ...
        results = []
        for profile in self.profiles:
            cookies_path = os.path.join(profile["Path"], "cookies.sqlite")
            if not os.path.exists(cookies_path): continue
            
            temp_db = os.path.join(os.environ["TEMP"], f"temp_ff_cookies_{os.getpid()}.sqlite")
            try:
                ...
                for row in cursor.fetchall():
                    host, path, name, value, expiry = row
                    results.append({
                        "host": host,
                        "path": path,
                        "name": name,
    ...
    def extract_history(self) -> List[Dict]:
        ...
        for profile in self.profiles:
            ...
            temp_db = os.path.join(os.environ["TEMP"], f"temp_ff_places_{os.getpid()}.sqlite")
            try:
                shutil.copy2(places_path, temp_db)
                import sqlite3
                conn = sqlite3.connect(temp_db)
                cursor = conn.cursor()
                cursor.execute("SELECT moz_places.url, moz_places.title, moz_places.visit_count, moz_historyvisits.visit_date FROM moz_places JOIN moz_historyvisits ON moz_places.id = moz_historyvisits.place_id")
                from datetime import datetime
                for row in cursor.fetchall():
                    url, title, count, visit_date = row
                    # visit_date is in PRTime (microseconds since epoch)
                    dt = datetime.fromtimestamp(visit_date / 1000000).strftime('%Y-%m-%d %H:%M:%S') if visit_date else "-"
                    results.append({
                        "url": url,
                        "title": title,
                        "visit_count": count,
    ...
    def extract_extensions(self) -> List[Dict]:
        self._get_profiles()
        results = []
        for profile in self.profiles:
            ext_path = os.path.join(profile["Path"], "extensions.json")
            ...
            try:
                with open(ext_path, 'r', encoding='utf-8') as f:
                    ...
                    for addon in addons:
                        ...
                        # Firefox perms are in userPermissions or permissions
                        perms = addon.get("userPermissions", {}).get("permissions", [])
