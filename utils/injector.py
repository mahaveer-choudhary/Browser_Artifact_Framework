    ...
    def inject_dll_via_early_bird(self, browser_path: str, dll_path: str) -> Optional[bytes]:
        ...
        try:
            ...
            def wait_pipe():
                ...
                if connected or self.kernel32.GetLastError() == 535: # ERROR_PIPE_CONNECTED
                    connection_result[0] = True

            pipe_thread = threading.Thread(target=wait_pipe)
            ...
            if not connection_result[0]:
                print("\n[!] CRITICAL ERROR: DLL failed to connect to the Named Pipe within 5 seconds.")
                print("    -> This happens when Windows Defender (or another AV) silently kills the Chrome ")
                print("       process because it detected the in-memory DLL injection.")
                print("    -> To run this tool, you must temporarily turn OFF 'Real-time protection' ")
                print("       or add an exclusion for this folder in Windows Security.\n")
                
                # Close the pipe so the daemon thread might unblock, and return Failure
                self.kernel32.CloseHandle(h_pipe)
                return None
